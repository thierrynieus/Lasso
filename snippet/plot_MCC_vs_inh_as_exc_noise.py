...
def plot_mcc(sim_num=1, idx_rng=range(1, 11)):
    ...
    for count, syn_type in enumerate(['all', 'exc', 'inh']):
        ...
        for i, (col, noise_type) in enumerate(zip(colors, noise_types)):
            ...
            m = mat.mean(axis=0)
            s = mat.std(axis=0)
            plt.errorbar(lamb, y=m, yerr=s, ecolor=col, color=col,
                         label=noise_labels[i], markersize=ms, elinewidth=2,
                         linewidth=2)
        ...
        plt.savefig(os.path.join(fpath_fig, '%s_%s.%s' % (nu.snum(sim_num),
                                                          syn_type, fig_ext)))
        plt.close()
